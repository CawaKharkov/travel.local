...
/**
 * Class CauserService
 * @package caUser\Entity
 */
class CauserService implements ServiceLocatorAwareInterface
{
    ...
    public function getRepository()
    {
        return $this->getEntityManager()->getRepository(
            ...
            ['doctrine']['authentication']['orm_default']['identity_class']
        );
    }
    ...
    {
        $authService = $this->serviceLocator->get(
        ...
        if ($authResult->isValid()) {
            ...
            return $redirectPlugin->toRoute(
                $this->causerConfig['options']['redirect']['route']
            );
        } else {
            return false;
        }
    ...
    {
        $authenticationService = $this->getServiceLocator()->get(
            'Zend\Authentication\AuthenticationService'
        );
    ...
    /**
     * @param string $email
     * @return $this->causerConfig['options']['redirect']['route']
     */
     ...
     * @return \caUser\Entity\User
     */
    public function registrationUser(\caUser\Entity\Base\EntityDefault $user)
    {
    ...
    public function exitUser()
    {
        ...
        $redirectPlugin = $pluginManager->get('redirect');
        return $redirectPlugin->toRoute('causer', ['action' => 'login']);
    }

    /**
     * Delete User
     * @param \caUser\Entity\Base\EntityDefault|\caUser\Entity\User $user
     * @return \caUser\Entity\UserService
     */
    public function deleteUser(\caUser\Entity\Base\EntityDefault $user)
    {
     ...
     * @param $email
     * @throws \Exception
     * @return \caUser\Entity\User|\ \caUser\Entity\Base\EntityDefault
     */
    public function restorePassword($email)
    {
        ...
        if ($user)
        {
            ...
            $transportOptions = new SmtpOptions(
                $this->causerConfig['password_restore']['mail']['smtp']
            );
            $message->setBody('Your new password: ' . $str);
            $message->addFrom($this->causerConfig
                ['password_restore']['mail']['options']['sender']
            );
            $message->addTo($email);
            $message->setSubject($this->causerConfig
                ['password_restore']['mail']['options']['subject']
            );
